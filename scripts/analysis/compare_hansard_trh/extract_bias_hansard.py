...
def normalize_stereotype_key(text: str) -> str:
    ...
    tokens = [t for t in tokens if t not in STOPWORDS_FOR_KEY]
    if tokens:
        return " ".join(tokens)
    return cleaned
...
def main() -> None:
    ...
    if "stance" in df.columns:
        before = len(df)
        df = df[df["stance"].astype(str).str.lower() != "irrelevant"].copy()
        dropped = before - len(df)
        if dropped:
            print(f"Filtered out {dropped} rows with stance='irrelevant'; {len(df)} remain.")

    if args.max_rows is not None:
        df = df.head(args.max_rows).copy()

    # Ensure a clean, 0..N-1 integer index so `row_id`
    # from enumerate(df.iterrows()) lines up with df.loc[row_id].
    ...
    # Add a count column (pre-dedup frequency) based on normalized_key/dimension/polarity.
    df_all = pd.DataFrame(all_records)
    if "normalized_key" not in df_all.columns:
        df_all["normalized_key"] = df_all["stereotype_text"].apply(normalize_stereotype_key)
    count_map = (
        df_all.groupby(["normalized_key", "dimension", "polarity"])
        .size()
        .reset_index(name="count")
    )
    if "normalized_key" not in df_out.columns:
        df_out["normalized_key"] = df_out["stereotype_text"].apply(normalize_stereotype_key)
    df_out = df_out.merge(count_map, on=["normalized_key", "dimension", "polarity"], how="left")
    df_out["count"] = df_out["count"].fillna(1).astype(int)

    # Drop normalized_key from the saved output.
    if "normalized_key" in df_out.columns:
        df_out = df_out.drop(columns=["normalized_key"])
