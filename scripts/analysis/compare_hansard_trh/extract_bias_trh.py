...
def normalize_stereotype_key(text: str) -> str:
    """Normalize stereotype text for deduplication (case/punct/stopwords)."""
...
def main():
    ...
    # Build a DataFrame to attach counts based on pre-dedup frequency.
    ...
    df = df.merge(df_counts, on=["normalized_key", "dimension", "polarity"], how="left")
    df["count"] = df["count"].fillna(1).astype(int)
    df = df.drop(columns=["normalized_key"])
    columns = ["chunk_id", "stereotype_text", "dimension", "polarity", "confidence", "count"]
    df = df[columns]
